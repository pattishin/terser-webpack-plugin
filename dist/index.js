...
class TerserPlugin {
  ...
  apply(compiler) {
    ...
    const optimizeFn = (compilation, chunks, callback) => {
      ...
      Array.from(chunks).reduce((acc, chunk) => acc.concat(chunk.files || []), []).concat(compilation.additionalChunkAssets || []).filter(_ModuleFilenameHelpers2.default.matchObject.bind(null, this.options)).forEach(file => {
        let inputSourceMap;
